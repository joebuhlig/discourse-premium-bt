	...
	class ExpirationWarning < ::Jobs::Scheduled
		...
		def execute(args)
			...
			group.users.each do |user|
				if user.custom_fields['premium_exp_date'] < Time.now + 40.days and user.custom_fields['premium_exp_pm_sent'] == false
					title = I18n.t("premium_bt.pm_warning_title")
					raw = I18n.t("premium_bt.pm_warning_text")
